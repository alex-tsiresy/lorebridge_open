...
export const WebsiteNode = memo(({ id, data, isConnectable, selected }: NodeProps) => {
  ...
  const getHandleStyle = useCallback((handleId: string) => {
    const color = NODE_COLORS.WEBSITE.handle;
    ...
    return baseStyle;
  }, [hoveredHandle, zoom, selected, isHandleNearCursor, id]);
  const stickyNoteStyle = getStickyNoteStyle('WEBSITE', selected);
  
  // Override styling to remove background and shadows for WebsiteNode
  const websiteNodeStyle = {
  ...
  return (
    ...
    >
      <NodeResizer
        color={NODE_COLORS.WEBSITE.border}
        isVisible={selected}
