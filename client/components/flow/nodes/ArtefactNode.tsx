...
import { ArtefactNodeContent } from './ArtefactNodeContent';
import { useNodeClickHandler } from '../hooks/useNodeClickHandler';
import { useConnection } from '../context/ConnectionContext';
import { getStickyNoteStyle } from '../utils';

export const ArtefactNode = memo(({ id, data, isConnectable, selected }: NodeProps) => {
  ...
  const getHandleStyle = (handleId: string) => {
    const color = NODE_COLORS.ARTEFACT.handle;
  ...
  const stickyNoteStyle = getStickyNoteStyle('ARTEFACT', selected);
  
  // Override styling to remove background and shadows for ArtefactNode
  const artefactNodeStyle = {
  ...
  return (
    ...
    >
      <NodeResizer
        color={NODE_COLORS.ARTEFACT.border}
        isVisible={selected}
        ...
        handleStyle={{
          ...
          border: `2px solid ${NODE_COLORS.ARTEFACT.border}`,
          borderRadius: '3px',
        }}
      />
...
ArtefactNode.displayName = 'ArtefactNode'; 