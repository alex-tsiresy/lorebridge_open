...
import { YouTubeNodeContent } from './YouTubeNodeContent';
...
export const YouTubeNode = memo(({ id, data, isConnectable, selected }: NodeProps) => {
  ...
  const getHandleStyle = (handleId: string) => {
    const color = NODE_COLORS.YOUTUBE.handle;
  ...
  const stickyNoteStyle = getStickyNoteStyle('YOUTUBE', selected);
  
  // Override styling to remove background and shadows for YouTubeNode
  const youtubeNodeStyle = {
  ...
  return (
    ...
    >
      <NodeResizer
        color={NODE_COLORS.YOUTUBE.border}
        isVisible={selected}
