...
export const DocumentNode = memo(({ id, data, isConnectable, selected }: NodeProps) => {
  ...
  const getHandleStyle = (handleId: string) => {
    const color = NODE_COLORS.DOCUMENT.handle;
  ...
  const stickyNoteStyle = getStickyNoteStyle('DOCUMENT', selected);
  
  // Override styling to remove background and shadows for DocumentNode
  const documentNodeStyle = {
  ...
  return (
    ...
    >
      <NodeResizer
        color={NODE_COLORS.DOCUMENT.border}
        isVisible={selected}
        ...
        handleStyle={{
          ...
          border: `2px solid ${NODE_COLORS.DOCUMENT.border}`,
          borderRadius: '3px',
        }}
      />
