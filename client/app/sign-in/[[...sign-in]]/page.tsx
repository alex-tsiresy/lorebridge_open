...
export default function Page() {
  ...
  async function handleEmailSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const emailFactorAny = (signIn.supportedFirstFactors || []).find(
        (f: any) => f?.strategy === "email_code"
      ) as any;
      const emailAddressId: string | undefined = emailFactorAny?.emailAddressId || emailFactorAny?.email_address_id;
      if (emailAddressId) {
        await signIn.prepareFirstFactor({ strategy: "email_code", emailAddressId });
        setPhase("verify");
        return;
      }
      setErrorMessage("Unable to sign in with this email address.");
    } catch (err: any) {
      const notFound = Array.isArray(err?.errors)
        && err.errors.some((er: any) => er?.code === "form_identifier_not_found");
      if (notFound) {
        setErrorMessage("No account found with this email address. Please sign up first or check your email.");
      } else {
        const message = err?.errors?.[0]?.message || "Unable to start sign in.";
  ...
  async function handleCodeSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErrorMessage("Additional steps required to sign in.");
    } catch (err: any) {
  ...
  async function continueWithFacebook() {
    ...
    try {
      ...
      const message = err?.errors?.[0]?.message || "Facebook sign-in failed.";
      setErrorMessage(message);
    }
  }
  ...
  async function continueWithApple() {
    ...
    try {
      ...
      const message = err?.errors?.[0]?.message || "Apple sign-in failed.";
      setErrorMessage(message);
    }
  }
