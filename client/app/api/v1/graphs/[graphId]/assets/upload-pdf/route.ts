...
// Configure larger body size limit for PDF uploads (20MB)
...
export async function POST(
  ...
  try {
    ...
    const backendUploadUrl = `${backendUrl}/api/v1/graphs/${graphId}/assets/upload-pdf`;
    ...
    if (!response.ok) {
      ...
      logger.error('Backend PDF upload error:', response.status, errorText);
    ...
    logger.error('Error in PDF upload route:', error);
    return NextResponse.json(
