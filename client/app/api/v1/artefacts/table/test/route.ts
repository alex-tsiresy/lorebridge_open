...
export async function POST(_request: NextRequest) {
  try {
    ...
    const backendTestUrl = `${backendUrl}/api/v1/artefacts/table/test`;
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Table test streaming error:', error);
    ...
    logger.error('Error in table test streaming route:', error);
    return NextResponse.json(
