...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendArtefactUrl = `${backendUrl}/api/v1/artefacts/table?${queryParams.toString()}`;
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Table artefact streaming error:', error);
    ...
    logger.error('Error in table artefact streaming route:', error);
    return NextResponse.json(
