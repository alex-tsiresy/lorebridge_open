...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendArtefactUrl = `${backendUrl}/api/v1/artefacts/graph?${queryParams.toString()}`;
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Graph artefact streaming error:', error);
    ...
    logger.error('Error in graph artefact streaming route:', error);
    return NextResponse.json(
