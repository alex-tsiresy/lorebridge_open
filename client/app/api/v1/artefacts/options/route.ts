...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendArtefactUrl = `${backendUrl}/api/v1/artefacts/options?${queryParams.toString()}`;

    // SSRF protection: allow-list upstream hosts
    const upstream = new URL(backendArtefactUrl);
    ...
    // Add Content-Type header if we have a body
    if (body) {
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Options artefact streaming error:', error);
    ...
    logger.error('Error in options artefact streaming route:', error);
    return NextResponse.json(
