...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendArtefactUrl = `${backendUrl}/api/v1/artefacts/document?${queryParams.toString()}`;
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Document artefact streaming error:', error);
    ...
    logger.error('Error in document artefact streaming route:', error);
    return NextResponse.json(
