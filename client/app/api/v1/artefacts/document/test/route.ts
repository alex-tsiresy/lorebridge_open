...
export async function POST(_request: NextRequest) {
  try {
    ...
    const backendTestUrl = `${backendUrl}/api/v1/artefacts/document/test`;
      ...
      start(controller) {
        ...
        function pump(): Promise<void> {
          return reader.read().then(({ done, value }) => {
            ...
            logger.error('Document test streaming error:', error);
    ...
    logger.error('Error in document test streaming route:', error);
    return NextResponse.json(
